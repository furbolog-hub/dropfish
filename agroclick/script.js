...
function openCell(id,source='normal'){
  ...
  if(Date.now()<suppressCellOpenUntil) return;
...
function catchRabbit(id){
  ...
  suppressCellOpenUntil=Date.now()+1600;
  rabbitTapGuard={cellId:caughtCellId,until:Date.now()+1600};
...
$('board').addEventListener('click',event=>{
  ...
  if(event.target.closest('[data-rabbit-id]')||Date.now()<suppressCellOpenUntil||(id!=null&&rabbitTapGuard.cellId===id&&Date.now()<rabbitTapGuard.until)){
    event.preventDefault();
    event.stopPropagation();
    event.stopImmediatePropagation();
  }
