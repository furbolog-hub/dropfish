...
function processBonus() {
  const name=pick(DATA.bonuses); state.bonusArtifactCount++;
  ...
  if (name==='Компас потерянных глубин') {
    if (!state.compassUsed) {
      ...
      showChoice('Компас потерянных глубин','Выберите новую погоду. Лимит забросов станет равен 10.',Object.keys(DATA.weather).map(k=>DATA.weather[k].name),choice=>{
        const key=Object.keys(DATA.weather).find(k=>DATA.weather[k].name===choice); state.weather=key; state.castsLeft=10; addHistory(`Компас сменил погоду на «${choice}» и восстановил лимит`,'epic'); render();
      });
...
function encounterAngus(fromCompass=false) {
  addHistory('🧔 Появился старина Ангус','event',fromCompass?'Призван повторным Компасом':'Случайная встреча');
  if (chance(.05)) {
...
function changeWeatherRandomly() {
  ...
  if (state.weather==='storm') state.stormSeen=true;
  addHistory(`Погода изменилась: ${DATA.weather[state.weather].name}`,'event');
}
...
function finishGame() {
  ...
  $('resultCard').innerHTML=`<h3>Итоговый вес: ${kg(total)}</h3><strong>Достижения:</strong>${earned.length?`<ul>${earned.map(x=>`<li>${x}</li>`).join('')}</ul>`:'<p>В этой сессии достижений нет.</p>'}<small>${ended.toLocaleString('ru-RU')}</small>`;
...
function renderHistory() {
  ...
  $('historyList').innerHTML=state.history.map((h,i)=>`<li class="history-item type-${h.type}"><strong>${i+1}. ${h.text}</strong>${h.detail?`<small>${h.detail}</small>`:''}</li>`).join('');
}
...
$('restartBtn').addEventListener('click',()=>{state=initialState();if(state.weather==='storm')state.stormSeen=true;$('resultCard').classList.add('hidden');$('resultCard').innerHTML='';render();toast('Началась новая игровая сессия');});
